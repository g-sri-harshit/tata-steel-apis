...
export function AIAnalysisCard({ agent, response, confidence, reasoning, impact, loading }: AIAnalysisProps) {
  ...
  const agentColor = AGENT_COLORS[agent] || "#6366f1";
    ...
    <div className="card-glass rounded-xl border border-steel-600/20 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-white text-sm">{agent} Agent Analysis</div>
            <div className="text-xs text-muted-foreground">Multi-agent AI · Gemini 1.5</div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {/* Confidence badge */}
