...
const REPORT_TABS: { key: ReportType; label: string; icon: React.ElementType; color: string }[] = [
  { key: "executive",   label: "Executive Summary", icon: BarChart3, color: "#6366f1" },
...
export default function ReportsPage() {
  ...
  const handlePdfExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `TataSteel_APIS_BoardReport_${Date.now()}.pdf`;
      a.click();
  ...
  const handleExport = (type: string) => {
    ...
    const a    = document.createElement("a");
    a.href = url; a.download = `apis-report-${type}-${Date.now()}.json`; a.click();
    URL.revokeObjectURL(url);
  };
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <ReportCard icon={Zap}         title="Energy Score"       value={ekpis.energy_score.toFixed(0)}              unit="/100" color="#3b82f6" delta="-0.8%" deltaPositive={false} />
                  <ReportCard icon={BarChart3}   title="Production KPI"     value={ekpis.production_kpi.toFixed(1)}            unit="%" color="#8b5cf6" delta="+5.3%" deltaPositive />
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <ReportCard icon={DollarSign} title="Total Cost"       value={`$${((maintData.total_cost_usd || 0) / 1000).toFixed(0)}k`}                                    color="#6366f1" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <Section title="Tasks by Type">
                    {maintTypeData.length > 0
                      ? <SimpleBarChart data={maintTypeData} xKey="name" bars={[{ key: "count", color: "#6366f1", label: "Tasks" }]} height={200} />
                      : <div className="h-48 flex items-center justify-center text-muted-foreground text-sm">Loading…</div>}
                  </Section>
                </div>
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <ReportCard icon={CheckCircle} title="Resolution Rate"   value={`${safetyData.resolution_rate?.toFixed(1) || "—"}%`}             color="#3b82f6" />
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <ReportCard icon={BarChart3} title="Total Consumption" value={`${((energyData.total_consumption_kwh || 0) / 1000).toFixed(0)}`} unit="MWh" color="#3b82f6" />
