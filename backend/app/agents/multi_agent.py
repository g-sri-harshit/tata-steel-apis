"""
Tata Steel APIS — Multi-Agent System
=====================================
...
if settings.GEMINI_API_KEY:
    ...
    _MODEL: Optional[genai.GenerativeModel] = genai.GenerativeModel("gemini-1.5-flash")
else:
    _MODEL = None
...
def _call_gemini(prompt: str, max_tokens: int = 1200) -> str:
    """Call Gemini with automatic rule-based fallback."""
    if _MODEL is None:
...
_SUPERVISOR_PROMPT = """You are the Supervisor Agent of the Tata Steel APIS (Autonomous Plant Intelligence System).
...
def _llm_classify_query(query: str) -> tuple[list[AgentRoute], dict[str, int]]:
    ...
    if _MODEL is not None:
        try:
            ...
            raw    = resp.text.strip().upper().replace(" ", "")
...
def maintenance_node(state: GraphState) -> GraphState:
    ...
    prompt = f"""
MAINTENANCE AGENT — TATA STEEL APIS
====================================
LIVE SENSOR CONTEXT:
...
def production_node(state: GraphState) -> GraphState:
    ...
    prompt = f"""
PRODUCTION AGENT — TATA STEEL APIS
=====================================
...
def reporting_node(state: GraphState) -> GraphState:
    ...
    prompt = f"""
REPORTING AGENT — TATA STEEL APIS
====================================
...
def _build_graph() -> StateGraph:
    """
    Construct and compile the APIS multi-agent LangGraph StateGraph.
...
logger.info("APIS LangGraph compiled successfully — 6 nodes, 7 edges")
